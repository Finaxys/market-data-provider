...
@XmlType(propOrder = {"mic", "symbol", "suffix", "provider", "name", "type", "continent", "country", "currency", "openTime", "closeTime", "status"})
public class Exchange implements Serializable{

	/**
	 * 
	 */
	private static final long serialVersionUID = -1456699782681111456L;
	...
	public Exchange(String mic, String symbol, String suffix, char provider, String name, String type, String continent,
			String country, String currency, DateTime openTime, DateTime closeTime, boolean status) {
		super();
		this.mic = mic;
		this.symbol = symbol;
		this.suffix = suffix;
	...
	public Exchange(byte[] mic, byte[] symbol, byte[] suffix, byte[] provider, byte[] name, byte[] type, byte[] continent,
		...
		this(Bytes.toString(mic), Bytes.toString(symbol), Bytes.toString(suffix),(char)(provider[0]), Bytes.toString(name), Bytes
				.toString(type), Bytes.toString(continent), Bytes.toString(country), Bytes.toString(currency), new DateTime(Bytes.toString(openTime)), new DateTime(Bytes.toString(closeTime)), Bytes.toBoolean(status));
	}
	 ...
	 */ 
	@XmlJavaTypeAdapter(com.finaxys.rd.marketdataprovider.domain.jaxb.TimeAdapter.class)	
